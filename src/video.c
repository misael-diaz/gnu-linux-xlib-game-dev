...
void vid_draw_gw(struct game const * const g)
{
	...
	for (int i = 0; i != g->entno; ++i) {
		...
		if (EN_GAMER == ent->tag) {
			...
			XSetForeground(g->display, g->gc, g->green.pixel);
		} else {
			// should never happen but we will know while playing the game
			XSetForeground(g->display, g->gc, WhitePixel(g->display, g->screeno));
		}
		XFillRectangle(
				g->display,
				g->window,
